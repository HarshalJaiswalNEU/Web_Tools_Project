...
//				.contentType(MediaType.parseMediaType("pdf")
...
//	}
	...
	public ModelAndView listCandidatesApplied(HttpServletRequest request,EmployerDAO employerDao) {
		...
		try {
			...
			Set<AppUsers> appUserInfof = new HashSet<>();
			...
			for(JobApplication ja : listApplication) {
				...
				for(AppUsers au: appUserInfo){
					if(uid.add(au.getUserid())){
						appUserInfof.add(au);
					}
				}
			}
			mav.addObject("listuser",appUserInfof.toArray());
			mav.addObject("listapp",listApplication);
			return mav;
		}catch (Exception e) {
			e.printStackTrace();
			System.out.println("Error in viewing candidate details");
			ModelAndView mavv = new ModelAndView("candidates-applied");
			return mavv;
		}
